...
class Calculator(QWidget):
    ...
    def buttonClicked(self):
        ...
        button = self.sender()
        key = button.text()
        special_num = ['3.141592', '3E+8', '340', '1.5E+8']
        fun_value = ['factorial', 'decTobin', 'binToDec', 'decToRoman']
        if key == '=':
            try:
            ...
            for i in range(0, 3):
                if key == constantList[i]:
                    self.display.setText(self.display.text() + special_num[i])
        elif key in functionList:
            for i in range(0, 3):
                if key == funtionList[i]:
                    n = self.display.text()
                    value = getattr(calcFuntions, fun_value[i])
                    self.display.setText(str(value(n)))
        else:
